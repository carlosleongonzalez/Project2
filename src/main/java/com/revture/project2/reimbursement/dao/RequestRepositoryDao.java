...
import com.revture.project2.reimbursement.entity.RequestEntity;
	...
	//-------------------------user Pending Request---------------------------------------------------------------------
		...
		@Query("SELECT rq FROM RequestEntity rq WHERE rq.usersId= ?1 AND rq.reqFlag=false AND rq.reject=false")
	...
	//--------------------------Specific Employee Requests---------------------------------------------------------------
	 ...
	 @Query("SELECT rq FROM RequestEntity rq WHERE rq.id= ?1 AND reqFlag=false AND reject=false")
